...
function accountCardHtml(a: typeof accountsTable.$inferSelect): string {
  ...
  return `<a class="card" href="/account/${escapeHtml(a.slug)}" style="${isSold ? "opacity:0.88;" : ""} position:relative;">
    ...
    <div class="card-img-wrap ${isRoyale ? "royale" : ""}" style="position:relative; width:100%; ${isRoyale ? "aspect-ratio:3/4; height:auto; background:#020617;" : "height:180px; aspect-ratio:16/9; background:#0f172a;"} overflow:hidden;">
      ${image && isRoyale ? `<img src="${escapeHtml(image)}" alt="" class="card-img-bg" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover; filter:blur(10px); transform:scale(1.15); opacity:0.35; pointer-events:none;" />` : ""}
      ${image ? `<img src="${escapeHtml(image)}" alt="${escapeHtml(a.title)}" class="card-img-main" loading="lazy" style="position:relative; width:100%; height:100%; ${isRoyale ? "object-fit:contain;" : "height:180px; object-fit:cover;"} display:block;" />` : ""}
